################################################################################################################
# gui.py        Version 3.8        13-Feb-2024     Bill Manaris, Dana Hughes, David Johnson, Kenneth Hanson, 
#                                                    and Autumn Lloyd.
...
#
# Copyright (C) 2014-2023 Bill Manaris, Dana Hughes, David Johnson, Kenneth Hanson, and Autumn Lloyd
#
...
# REVISIONS:
#
#   3.8     13-Feb-2023 (al) Added ArcCircle class and HALF_PI, PI, and TWO_PI constants. This
#                  		allows creation of perfectly circular Arcs given a center point,
#                       radius, start angle, end angle, and stylistic parameters. Also, added
#						CHORD, OPEN, and PIE fill styles on Arc and ArcCircle.
#
#   3.7     22-Apr-2020 (bm)  Added capability for gradient color.  Also, pasted code for guicontrols 
#                       at the bottom (instead if importing them - simplifies things).
#
...
# hidden (internal) helper function
def __processColorOrGradient__(color):
   """
   Returns either a java.awt.Color or a java.awt.GradientPaint object, based on the input parameter.
   Both are usable in painting aribitrary Java Graphics2D shapes.
   """
   ...
   elif (type(color) is list) and len(color) == 6:
   	try:
   	   color = GradientPaint( *color )   # try to create gradient
   	except:
   	   raise TypeError("Expected [x1, y1, color, x2, y2, color]... (got " + str( color ) + " instead).")	

   # check if java.awt.Color
   elif not isinstance(color, Color):
      raise TypeError("Expected a color... (got " + str( color ) + " instead).")	
...
class Drawable():
   ...
   def setColor(self, color=None):
      ...
      # make sure color is either a java.awt.Color or a java.awt.GradientPaint object
      self.color = __processColorOrGradient__(color)
      if self.display:
...
class Arc(JPanel, Widget, Drawable):
   ...
   def __init__(self, x1, y1, x2, y2, startAngle = PI, endAngle = TWO_PI, style = OPEN, color = Color.BLACK, fill = False, thickness=1):
      """
      Create a new arc
      """
   ...
   def paint(self, graphics2DContext):
      ...
      if self.style == OPEN:
         tempArc = Arc2D.Float(Arc2D.OPEN)
         tempArc.setFrame(self.startX_JPanel, self.startY_JPanel, self.endX_JPanel, self.endY_JPanel)
         tempArc.setAngleStart(self.startAngle)
         tempArc.setAngleExtent(self.arcAngle)
         graphics2DContext.setColor(self.color)
         graphics2DContext.draw(tempArc)
      elif self.style == CHORD: 
         tempArc = Arc2D.Float(Arc2D.CHORD)
         tempArc.setFrame(self.startX_JPanel, self.startY_JPanel, self.endX_JPanel, self.endY_JPanel)
         tempArc.setAngleStart(self.startAngle)
         tempArc.setAngleExtent(self.arcAngle)
         graphics2DContext.setColor(self.color)
      ...
      # do we need to fill the arc?
      if self.fill:    
      	graphics2DContext.fill(tempArc)
...
# ArcCircle
#
# Creates a circular Arc to be added to a display.

class ArcCircle(Arc):
   """
   An arc specified by a center point, radius, the start angle, and the end angle.  Angles are interpreted such that 0 degrees is 
   at the three o'clock position. A positive value indicates a counter-clockwise rotation while 
   a negative value indicates a clockwise rotation. The constants HALF_PI, PI, and TWO_PI can be used in place of degrees.
   """
   def __init__(self, x, y, radius, startAngle = PI, endAngle = TWO_PI, style = PIE ,color = Color.BLACK, fill = False, thickness = 1):
      """
      Create a new ArcCircle based off of the Arc Class
      """
      # calcualting x1, y1, x2, y2 based off of center
      x1 = x - radius
      y1 = y - radius
      x2 = x + radius
      y2 = y + radius
      ...
      # Inherit from Arc.
      Arc.__init__(self, x1, y1, x2, y2, startAngle, endAngle, style, color, fill, thickness)
...
# include additional GUI controls (new as of v3.7) 
